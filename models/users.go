...
type Users struct {
	...
	Customer      *Customers `orm:"rel(fk);column(customer_id);null"`
...
// GetAllUsers retrieves all Users matches certain condition. Returns empty list if
// no records exist
func GetAllUsersByBranch(branch *Branches, query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Users))
	// query k=v
