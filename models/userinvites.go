...
// no records exist
func GetAllUserInvitesByEmail(email string, query map[string]string, fields []string, sortby []string, order []string,
	...
	if len(sortby) != 0 {
		if len(sortby) == len(order) {
			...
			qs = qs.Filter("Email", email).OrderBy(sortFields...)
	...
	var l []UserInvites
	qs = qs.Filter("Email", email).OrderBy(sortFields...).RelatedSel()
